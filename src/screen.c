...
void received_msg_Details() {
  ...
  Paint_DrawString_EN(0, 25, saved_Messages[received_Cursor + ((received_Page - 1) * 3)], &Font16,
                      BLACK, WHITE);
...
void received_Msgs() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (i + ((received_Page - 1) * 3) < msg_Number) {
      ...
      Paint_DrawString_EN(20, 34 + i * 24, saved_Messages[i + ((received_Page - 1) * 3)], &Font16,
                          BLACK, WHITE);
      // sprintf(new_String, "From: %s",node_ID); //Get node ID from network code
      Paint_DrawString_EN(20, 47 + i * 24, "From: 11.22.33", &Font12, BLACK,
                          WHITE); // replace with new_String
      // sprintf(new_String, "%s mins ago",time_Stamp); //Get time_Stamp from network code
      Paint_DrawString_EN(130, 47 + i * 24, "3 mins ago", &Font12, BLACK,
                          WHITE); // replace with new_String
    }
